...
#p.pozzobon mostra il lavoro svolto nel lato frontend, mostrando i requisiti implementati e le funzionalità aggiunte. Fa notare come alcune delle funzionalità implementate non erano presenti nella prima scrittura del documento #glossario("AdR") e che sono state aggiunte in seguito ad una discussione con il team. Presenta come prossimi passi il continuamento degli ultimi requisiti da implementare e il collegamento del lato front-end con quello di back-end per il reperimento dei dati (menù già predisposto) \ Alex Beggiato si ritiene soddisfatto del lavoro svolto e consiglia di aumentare di poco lo spacing tra le barre verticali del grafico, per aiutare l'utente a selezionare quelle barre che sono più \"interne\" al grafico e di conseguenza più di difficile selezione. \

== Domande e chiarimenti
#p.lucato chiede ad Alex Beggiato alcune domande riguardanti l'interfaccia _MatrixData_ e la sua implementazione. \ Espone i suoi dubbi riguardo all'ultima riunione effettuata mostrando in contemporanea l'implementazione che sembrava più coerente con quanto discusso in precendenza. \ Alex Beggiato conferma che l'implementazione mostrata da #p.lucato è corretta e che il lavoro svolto è in linea con quanto discusso. Portare l'oggetto MatrixData ad essere un'interfaccia e implementarla a seconda dell'oggetto di ritorno necessario (in base al tipo di reperimento di dati), permette di avere sempre un'unica interfaccia di ritorno nelle varie funzioni implementate, con la possibilità di aggiungere altre variabili in una futura implementazione, rendendo il codice più robusto e manutenibile.\

== Prossimi passi
Come indicato da Alex Beggiato, verranno implementati i test di unità per il lato back.end. Anche nel lato frontend verranno implementati i test di unità per le funzionalità già implementate, con la possibilità di implementare anche i test e2e. Si procederà dunque con il collegamento del front-end con il back-end per il reperimento dei dati con le successive funzioni per il parsing dei dati. \
Si iniziano i lavori per la scrittura del documento della #glossario("ST") e l'inserimento dei nuovi requisiti nel documento #glossario("AdR"). \

== Varie ed eventuali
Non essendoci altri argomenti da discutere l’incontro è terminato alle 16:40. \
Il prossimo incontro #glossario("SAL") è stato programmato per il giorno 04/04/2025 con orario da stabilire in base alle disponibiltà del team e dell'azienda proponente \
])

= Decisioni

#decisioni((
  [1],[Test di un'unità],[Implementare ulteriori test di unità in alcune classi per aumentare la percentuale di test di mutazione],
  [2],[Collegamento front-end con back-end],[Collegare il front-end con il back-end per il reperimento dei dati],
  [3],[Test di unità],[Implementare test di unità per le funzionalità già implementate nel lato frontend],
