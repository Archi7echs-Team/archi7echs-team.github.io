...
#show: conf.with(
  ...
  outline_depth: 2,
  changelog: (
    "1.4.0","11-04-2025","Modifica numerazione e dettaglio req. funzionali. Aggiornamento test di unità. Aggiunta descrizione alle tabelle dei test",p.pozzobon,"",
...
#align(center)[
  ...
  [T.S.31], [Verificare che l'utente possa visualizzare i soli dati che sono contenuti all'interno di un intervallo di valori (che può essere chiuso o aperto), opacizzando quindi gli altri],[F.1.27] ,[S],
  [T.S.32], [Verificare che l'utente possa visualizzare le sole barre con altezza compresa tra le _n_ altezze maggiori, con _n_ definito dall'utente, opacizzando, di conseguenza, le barre con altezza esclusa da questo intervallo],[F.1.28] ,[S],
  [T.S.33], [Verificare che l'utente possa visualizzare le sole barre con altezza compresa tra le _n_ altezze minore, con _n_ definito dall'utente, opacizzando, di conseguenza, le barre con altezza esclusa da questo intervallo],[F.1.29] ,[S],
  [T.S.34], [Verificare che l'utente possa scegliere il metodo di colorazione del grafico],[F.1.30] ,[S],
  [T.S.35], [Verificare che l'utente possa eliminare tutti i filtri applicati al grafico, riportandolo allo stato di default, tramite un apposito bottone],[F.1.31] ,[S],
  [T.S.36], [Verificare che l'utente possa esportare tutte le informazioni relative ad un grafico a cui si sta lavorando, ovvero l'insieme di dati ed i filtri relativi],[F.2.1 - Desiderabile] ,[NI],
  [T.S.37], [Verificare che l'utente possa esportare un'istantanea del grafico],[F.2.2 - Desiderabile] ,[S],
  [T.S.38], [Verificare che l'utente venga avvertito graficamente nel caso abbia inserito un input non conforme alle aspettative],[F.1.32] ,[S],
  [T.S.39], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a caricare un quantitativo di dati (manualmente o automaticamente) che superano il numero massimo supportato],[F.1.33] ,[S],
  [T.S.40], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a selezionare un file .csv con la formattazione interna non corretta],[F.1.34] ,[S],
  [T.S.41], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a caricare dei dati tramite chiamata ad un #glossario("API") esterna che però non ha risposto alla richiesta entro un tempo limite],[F.1.35] ,[S],
  [T.S.42], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a lasciare un input vuoto, quando invece è richiesto obbligatoriamente un valore],[F.1.36 - Opzionale] ,[S],
  [T.S.43], [Verificare che l'utente venga avvertito graficamente nel caso in cui la connessione al database locale sia fallita],[F.1.37] ,[S],
  [T.S.44], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a selezionare un file .csv con un peso che supera il limite consentito],[F.1.38] ,[S],
  [T.S.45], [Verificare che l'utente possa accedere direttamente alla pagina dell'applicazione senza sistema di login],[F.1.39] ,[S],
  [T.S.46], [Verificare che l'applicazione possa elaborare dati con coordinate X, Y e Z, dove X e Z definiscono il posizionamento della base della barra nel piano mentre Y definisce l’altezza della barra],[F.1.40] ,[S],
  [T.S.47], [Verificare che l'applicazione deve essere in grado di supportare una quantità massima di dati pari a 1000],[V.1.1] ,[S],
  [T.S.48], [Verificare che l'applicazione possa supportare  un massimo di 300 elementi per le coordinate X e Z mantenendo il limite di 1000 dati],[V.1.2] ,[S],
  [T.S.49], [Verificare che l'applicazione possa supportare il caricamento dati da un file .csv che pesa al massimo 10 MB],[V.1.3] ,[S],
  )
    <tab:testdisistema>
]
...
#align(center)[
  ...
  [T.U.58], [Verifica che la funzione iSt logghi un errore se la fetch per l'API esterna fallisce], [S],
  [T.U.59], [Verifica che tutti i componenti di DataFilter.svelte vengano caricati correttamente, inclusi i testi di "Visualization interval", "Show average plane", "Values lower than the global average", "Values higher than the global average", e "Visualization reset"], [S],
  [T.U.60], [Verifica che avgFilter venga impostato su 1 quando viene cliccato il pulsante "Lower than average" in DataFilter.svelte], [S],
  [T.U.61], [Verifica che avgFilter e avgEnabled vengano impostati su 0 quando viene cliccato il pulsante "Reset" in DataFilter.svelte], [S],
  [T.U.62], [Verifica che avgFilter venga impostato su 2 quando viene cliccato il pulsante "Greater than average" in DataFilter.svelte], [S],
  [T.U.63], [Verifica che la checkbox "Show average plane" sia selezionabile e che avgEnabled venga impostato correttamente in DataFilter.svelte], [S],
  [T.U.64], [Verifica che tutti i componenti di DataSource.svelte vengano caricati correttamente, inclusi i testi di "External API", "DB1", "File" e "Send file CSV"], [S],
  [T.U.65], [Verifica che le funzioni fetchExternal e resetCamera vengano chiamate quando viene cliccato il pulsante "Select API" in DataSource.svelte], [S],
  [T.U.66], [Verifica che le funzioni fetchDb e resetCamera vengano chiamate quando viene cliccato il pulsante "Select DB1" in DataSource.svelte], [S],
  [T.U.67], [Verifica che la funzione uploadFile venga chiamata quando viene cliccato il pulsante "Select CSV" in DataSource.svelte], [S],
  [T.U.68], [Verifica che il componente Export.svelte venga renderizzato correttamente senza errori], [S],
  [T.U.69], [Verifica che tutti i componenti di Export.svelte vengano caricati correttamente, inclusi i testi di "Export as image" e "Screenshot"], [S],
  [T.U.70], [Verifica che la funzione takeScreenshot venga chiamata quando viene cliccato il pulsante "Screenshot" in Export.svelte], [S],
  [T.U.71], [Verifica che la funzione getValueFromId restituisca il valore corretto per un dato id], [S],
  [T.U.72], [Verifica che la funzione selectionToggle aggiunga e rimuova correttamente l'id dall'array di selezione], [S],
  [T.U.73], [Verifica che il valore predefinito venga caricato correttamente nelle impostazioni di filtro], [S],
  [T.U.74], [Verifica che la funzione lastValue restituisca 0 se non ci sono barre selezionate], [S],
  [T.U.75], [Verifica che la funzione lastValue restituisca il valore dell'ultima barra selezionata], [S],
  [T.U.76], [Verifica che la funzione set sostituisca correttamente la selezione esistente], [S],
  [T.U.77], [Verifica che la funzione set svuoti la selezione quando viene passata un array vuoto], [S],
  [T.U.78], [Verifica che il filtro venga inizializzato con i valori predefiniti corretti], [S],
  [T.U.79], [Verifica che la funzione resetFilter ripristini i valori del filtro ai valori predefiniti corretti], [S],
  [T.U.80], [Verifica che la variabile rangeValue venga aggiornata correttamente], [S],
  [T.U.81], [Verifica che la variabile avgEnabled venga modificata correttamente], [S],
  [T.U.82], [Verifica che la variabile colorSelection venga aggiornata correttamente], [S],
  [T.U.83], [Verifica che la funzione getSelectedBarInfo restituisca null quando nessuna barra è selezionata], [S],
  [T.U.84], [Verifica che la funzione getSelectedBarInfo restituisca le informazioni corrette della barra quando una barra è selezionata], [S],
  [T.U.85], [Verifica che la funzione takeScreenshot chiami correttamente i metodi di rendering e salvataggio dell'immagine], [S],
  [T.U.86], [Verifica che la funzione downloadImage crei correttamente un link e triggeri il click per il download dell'immagine], [S],
  [T.U.87], [Verifica che la funzione resetCamera ripristini correttamente la posizione della telecamera], [S],
  [T.U.88], [Verifica che la funzione zoomIn modifichi correttamente la posizione della telecamera], [S],
  [T.U.89], [Verifica che la funzione zoomOut modifichi correttamente la posizione della telecamera], [S],
  [T.U.90], [Verifica che la funzione setBarFilterSelection imposti filter.barFilterSelection solo se il valore è ammesso], [S],
  [T.U.91], [Verifica che filter.selection.clear() venga chiamato da setBarFilterSelection quando il valore passato è 0], [S],[T.U.92], [Verifica che filter.selection.clear() non venga chiamato da setBarFilterSelection se il valore passato è diverso da 0], [S],
  [T.U.93], [Verifica che la funzione hideBarFilterPane imposti displayBarFilter a false], [S],
  [T.U.94], [Verifica che la funzione isInRange restituisca true se l'altezza è compresa nel range], [S],
  [T.U.95], [Verifica che la funzione isInRange restituisca false se l'altezza è al di fuori del range], [S],
  [T.U.96], [Verifica che la funzione isInRange restituisca false se min e max sono uguali], [S],
  [T.U.97], [Verifica che la funzione passesBarFilter restituisca true se la barra passa il filtro con avgFilter == 0], [S],[T.U.98], [Verifica che la funzione passesBarFilter restituisca false se la barra non passa il filtro con avgFilter == 1], [S],
  [T.U.99], [Verifica che la funzione passesBarFilter restituisca false se la barra non passa il filtro con avgFilter == 2], [S],
  [T.U.100], [Verifica che la funzione passesBarFilter restituisca false se la barra non passa il filtro con barFilterSelection == 2], [S],
  [T.U.101], [Verifica che la funzione passesBarFilter restituisca false se la barra non passa il filtro con barFilterSelection == 3], [S],
  [T.U.102], [Verifica che la funzione passesBarFilter restituisca true se nessun filtro è applicato], [NI],
  [T.U.103], [Verifica che la funzione getBarColor restituisca il colore predefinito se colorSelection è 0], [S],
  [T.U.104], [Verifica che la funzione getBarColor restituisca un colore basato sulla coordinata x quando colorSelection è 1], [S],[T.U.105], [Verifica che la funzione getBarColor restituisca un colore basato sulla coordinata z quando colorSelection è 2], [S],
  [T.U.106], [Verifica che la funzione getBarColor restituisca un colore basato sul valore di altezza quando colorSelection è 3], [S],
  [T.U.107], [Verifica che la funzione isFirstIntersected restituisca true se l'oggetto è intersecato], [S],
  [T.U.108], [Verifica che la funzione isFirstIntersected restituisca false se l'oggetto non è intersecato], [S],
  [T.U.109], [Verifica che la funzione handleTextClick non faccia nulla se l'oggetto non è intersecato], [S],
  [T.U.110], [Verifica che la funzione uploadFile aggiorni fetchedData se uploadCsvFile restituisce dati validi], [S],
  [T.U.111], [Verifica che la funzione uploadFile non modifichi fetchedData se uploadCsvFile restituisce null], [S],
  [T.U.112], [Verifica che la funzione uploadFile chiami console.error quando si verifica un errore durante il caricamento del file], [S],
  [T.U.113], [Verifica che la funzione sortAscData ordini i dati in modo crescente e rimuova i duplicati], [S],
  [T.U.114], [Verifica che la funzione sortAscData restituisca un array vuoto se i dati sono vuoti], [S],
  [T.U.115], [Verifica che la funzione sortAscData mantenga gli elementi unici], [S],
  [T.U.116], [Verifica che la funzione sortDescData ordini i dati in modo decrescente e rimuova i duplicati], [S],
  [T.U.117], [Verifica che la funzione sortDescData restituisca un array vuoto se i dati sono vuoti], [S],
  [T.U.118], [Verifica che la funzione sortDescData mantenga gli elementi unici], [S],
  [T.U.119], [Verifica che la funzione fetchDb aggiorni fetchedData con i dati dal server se disponibili], [S],
  [T.U.120], [Verifica che la funzione fetchDb non aggiorni fetchedData se getDbData restituisce null], [S],
  [T.U.121], [Verifica che la funzione fetchExternal aggiorni fetchedData con i dati dal server se disponibili], [S],
  [T.U.122], [Verifica che la funzione fetchExternal non aggiorni fetchedData se getExternalData restituisce null], [S],
  [T.U.123], [Verifica che la funzione isFirstIntersected restituisca true se l'oggetto è intersecato], [S],
  [T.U.124], [Verifica che la funzione isFirstIntersected restituisca false se l'oggetto non è intersecato], [S],
  [T.U.125], [Verifica che il componente Scene.svelte venga renderizzato correttamente senza errori], [S],
  [T.U.126], [Verifica che il componente SettingsPane.svelte venga renderizzato correttamente senza errori], [S],
  [T.U.127], [Verifica che tutti i componenti di SettingsPane.svelte vengano caricati correttamente, inclusi i testi di "Settings", "Camera", "Source", "Filter", "Color" e "Export"], [S],
  )
    <tab:testdiunita>
]
