...
#align(center)[
  ...
  [T.S.29], [Verificare che l'utente possa inserire il valore $n$ così da visualizzare gli $n$ valori maggiori o minori],[F.1.25] ,[NI],
  [T.S.30], [Verificare che l'utente possa visualizzare all'interno del grafico gli $n$ dati (barre) con valore più alto, con $n$ da lui inserito],[F.1.26] ,[NI],
  [T.S.31], [Verificare che l'utente possa visualizzare all'interno del grafico gli $n$ dati (barre) con valore più basso, con $n$ da lui inserito],[F.1.27] ,[NI],
  [T.S.32], [Verificare che l'utente possa eliminare tutti i filtri applicati al grafico, riportandolo allo stato di default, tramite un apposito bottone],[F.1.28] ,[NI],
  [T.S.33], [Verificare che l'utente possa esportare tutte le informazioni relative ad un grafico a cui si sta lavorando, ovvero l'insieme di dati ed i filtri relativi],[F.2.1 - Desiderabile] ,[NI],
  [T.S.34], [Verificare che l'utente possa effettuare uno "screenshot" del grafico],[F.2.2 - Desiderabile] ,[NI],
  [T.S.35], [Verificare che l'utente venga avvertito graficamente nel caso abbia inserito un input non conforme alle aspettative],[F.1.29] ,[NI],
  [T.S.36], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a caricare un quantitativo di dati (manualmente o automaticamente) che superano il numero massimo supportato],[F.1.30] ,[NI],
  [T.S.37], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a selezionare un file .csv con la formattazione interna non corretta],[F.1.31] ,[NI],
  [T.S.38], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a caricare dei dati tramite chiamata ad un #glossario("API") esterna che però non ha risposto alla richiesta entro un tempo limite],[F.1.32] ,[NI],
  [T.S.39], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a lasciare un input vuoto, quando invece è richiesto obbligatoriamente un valore],[F.1.33] ,[NI],
  [T.S.40], [Verificare che l'utente venga avvertito graficamente nel caso in cui la connessione al database locale sia fallita],[F.1.34] ,[NI],
  [T.S.41], [Verificare che l'utente venga avvertito graficamente nel caso abbia provato a selezionare un file .csv con un peso che supera il limite consentito],[F.1.35] ,[NI],
  [T.S.42], [Verificare che l'utente possa accedere direttamente alla pagina dell'applicazione senza sistema di login],[F.1.36] ,[NI],
  [T.S.43], [Verificare che l'applicazione possa elaborare dati con coordinate X, Y e Z, dove X e Z definiscono il posizionamento della base della barra nel piano mentre Y definisce l’altezza della barra],[F.1.37] ,[NI],
  [T.S.44], [Verificare che l'applicazione deve essere in grado di supportare una quantità massima di dati pari a 1000],[V.1.1] ,[NI],
  [T.S.45], [Verificare che l'applicazione possa supportare  un massimo di 300 elementi per le coordinate X e Z mantenendo il limite di 1000 dati],[V.1.2] ,[NI],
  [T.S.46], [Verificare che l'applicazione possa supportare il caricamento dati da un file .csv che pesa al massimo 10 MB],[V.1.3] ,[NI],
  )
    <tab:testdisistema>
]
