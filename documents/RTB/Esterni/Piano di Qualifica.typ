...
#show: conf.with(
  ...
  outline_depth: 2,
  changelog: (
    "0.3.2", "31-01-2025", "Aggiunta termini glossario", p.scandaletti, "",
...
#align(center)[
  ...
  [T.S.8], [Verificare che l'utente possa inserire automaticamente i dati tramite una connessione a #glossario("database") #glossario("SQL") come metodo di caricamento],[F.1.4] ,[NI],
  ...
  [T.S.16], [Verificare che l'utente, premendo sopra ad una barra, visualizzi una sezione apposita per scegliere tra alcune delle ulteriori #glossario("funzionalità") ],[F.1.12] ,[NI],
  ...
  [T.S.19], [Verificare che l'utente, una volta premuta una barra, abbia la possibilità di #glossario("opacizzare") tutti i dati che hanno valore minore del valore della barra selezionata],[F.1.15] ,[NI],
  [T.S.20], [Verificare che l'utente, una volta premuta una barra, abbia la possibilità di #glossario("opacizzare") tutti i dati che hanno valore maggiore del valore della barra selezionata],[F.1.16] ,[NI],
  ...
  [T.S.22], [Verificare che l'utente possa attivare/disattivare la visualizzazione di un #glossario("piano parallelo alla base") del grafico, rappresentante il valore medio globale],[F.1.18] ,[NI],
  [T.S.23], [Verificare che l'utente possa #glossario("opacizzare") i soli dati che sono minori della media globale, tramite un apposito bottone],[F.1.19] ,[NI],
  [T.S.24], [Verificare che l'utente possa #glossario("opacizzare") i soli dati che sono maggiori della media globale, tramite un apposito bottone],[F.1.20] ,[NI],
  [T.S.25], [Verificare che l'utente possa visualizzare i soli dati che sono contenuti all'#glossario("interno") di un intervallo di valori (che può essere chiuso o aperto), opacizzando quindi gli altri],[F.1.21] ,[NI],
  [T.S.26], [Verificare che l'utente possa visualizzare il #glossario("piano parallelo alla base") che identifica la media dei valori appartenenti all'asse X selezionato],[F.1.22] ,[NI],
  [T.S.27], [Verificare che l'utente possa visualizzare il #glossario("piano parallelo alla base") che identifica la media dei valori appartenenti all'asse Z selezionato],[F.1.23] ,[NI],
  [T.S.28], [Verificare che l'utente può visualizzare all'#glossario("interno") del grafico gli N dati (barre) con valore più alto, opacizzando gli altri],[F.1.24] ,[NI],
  [T.S.29], [Verificare che l'utente può visualizzare all'#glossario("interno") del grafico gli N dati (barre) con valore più basso, opacizzando gli altri],[F.1.25] ,[NI],
...
#align(center)[
  ...
  [T.A.7], [Verificare che l'utente possa caricare i dati tramite una connessione ad un #glossario("database") #glossario("SQL")], [NI],
  [T.A.8], [Verificare che l'utente possa caricare i dati tramite una connessione ad un #glossario("API") #glossario("esterno")], [NI],
